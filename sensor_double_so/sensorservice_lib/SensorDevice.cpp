...
//fsz ++
	...
	if(lib == NULL)
	{
		ALOGW("WSLIB SensorDevice  add /system/lib/libws_sensor.so err\n");
	}else
	{
		ALOGW("WSLIB SensorDevice  add /system/lib/libws_sensor.so suc\n");
		wsdevice = (struct ws_device_t *)malloc(sizeof(struct ws_device_t));
		if(wsdevice==NULL)
		{
		...
		{
			ALOGW("WSLIB  malloc  device success\n");
			wsdevice->Init_WsSensorDevice = (int (*)(struct ws_device_t **wsdevice,void *lib))(dlsym(lib,"Init_WsSensorDevice"));
			int version = wsdevice->Init_WsSensorDevice(&wsdevice,lib);
			ALOGW("WSLIB fun ret = %d \n",version);
		}
	}
//fsz ++
...
ssize_t SensorDevice::getSensorList(sensor_t const** list) {
	...
	if(wsdevice!=NULL)
	{
		countEx = wsdevice->getSensorListEx(mSensorModule,list);//fsz ++
		if(countEx > 0) return countEx;
	}
	//hzl.add end
...
status_t SensorDevice::initCheck() const {
	...
	if(wsdevice!=NULL)
	{
		if(wsdevice->initCheckEx()) return NO_ERROR;
	}
	//hzl.add end
...
status_t SensorDevice::activate(void* ident, int handle, int enabled)
{
	...
	if(wsdevice!=NULL)
	{
		if(wsdevice->sensorHandleCheck(handle)) return NO_ERROR;
	}
	//hzl.add end
