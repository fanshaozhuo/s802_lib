...
void SensorService::onFirstRef()
{
    ...
    if (dev.initCheck() == NO_ERROR) {
        ...
        if (count > 0) {
			...
			sp<WS_Thread>  thread = new WS_Thread(this);//ljj
    		thread->run("WS_Thread", PRIORITY_URGENT_DISPLAY);
...
bool SensorService::threadLoop()
{
    ...
    do {
        count = device.poll(buffer, numEventMax);//fsz --
       //count = device.wsdevice->SensorDevicePoll(buffer,count);//fsz ++
        if (count<0) {
        ...
        //device.wsdevice->DumpGsensordata(buffer,count);//fsz ++
...
// ---------------------------------------------------------------------------
SensorService::WS_Thread::WS_Thread(const sp<SensorService>& service) :
        mService(service),Thread(false) {
    ALOGW("WS_Thread");
}

bool SensorService::WS_Thread::threadLoop() {
	ALOGD("WS_Thread threadLoop starting...");
//	ALOGD("WS_Thread threadLoop starting...");
    ALOGE("WS_Thread threadLoop starting...");
	// each virtual sensor could generate an event per "real" event, that's why we need
	// to size numEventMax much smaller than MAX_RECEIVE_BUFFER_EVENT_COUNT.
	// in practice, this is too aggressive, but guaranteed to be enough.
	const size_t minBufferSize = SensorEventQueue::MAX_RECEIVE_BUFFER_EVENT_COUNT;
	const size_t numEventMax = minBufferSize / (1 + mService->mVirtualSensorList.size());

	sensors_event_t buffer[minBufferSize];
	sensors_event_t scratch[minBufferSize];
	SensorDevice& device(SensorDevice::getInstance());
	const size_t vcount = mService->mVirtualSensorList.size();
	...
	do {
	   ...
	   count = device.wsdevice->SensorDevicePoll(buffer,count);//fsz ++
		if (count<0) {
			ALOGE("WS_Thread sensor poll failed (%s)", strerror(-count));
			break;
		}
		...
		mService->recordLastValue(buffer, count);
		//device.wsdevice->DumpGsensordata(buffer,count);//fsz ++
		// handle backward compatibility for RotationVector sensor
		if (halVersion < SENSORS_DEVICE_API_VERSION_1_0) {
			for (int i = 0; i < count; i++) {
		...
		for (size_t i=0 ; i<numConnections ; i++) {
			...
			if (connection != 0) {
				connection->sendEvents(buffer, count, scratch);
				// Some sensors need to be auto disabled after the trigger
				mService->cleanupAutoDisabledSensor(connection, buffer, count);
			}
		}
    ...
    ALOGW("Exiting WS_Thread  threadLoop => aborting...");
    abort();
    return false;
}

SensorService::WS_Thread::~WS_Thread(){
    ALOGW("~~~~~~WS_Thread");
}
