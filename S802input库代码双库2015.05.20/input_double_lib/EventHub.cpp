...
const char * const     WS_SENSOR_LIB_PATH = "/system/lib/libwsinput.so";//fsz ++
struct WSDEVICE{
	int (*fun)(void);
...
static bool getAbsoluteAxisInfoEx(int32_t deviceId, int axis, RawAbsoluteAxisInfo* outAxisInfo){
	int i =0;
	for(i=0;i<mJoyCount;i++)
	{

		ALOGE("<<<<<<LEE >>>>>> getAbsoluteAxisInfo  %d !!!!!!!!!!!",i);
		if(mJoystickDeviceId[i] == deviceId)
		{
			switch (axis)
			{
...
EventHub::EventHub(void) :
    ...
    LOG_ALWAYS_FATAL_IF(result != 0, "Could not add wake read pipe to epoll instance.  errno=%d",
            errno);
	memset(mJoystickDeviceId, 0, sizeof(mJoystickDeviceId));
	void *lib ;
	int ret;
	ret = -1;
	wsdevice = NULL;
	wsdevice = (struct WSDEVICE *)malloc(sizeof(struct WSDEVICE));
	if(wsdevice==NULL){
		ALOGW("WSLIB  malloc  device failure\n");
	}
	lib = dlopen(WS_SENSOR_LIB_PATH,RTLD_NOW);
	if(lib == NULL){
		ALOGW("WSLIB SensorDevice  add /system/lib/libws_input err\n");
	}else{
		ALOGW("WSLIB SensorDevice  add /system/lib/libws_input suc\n");
	}
	wsdevice->fun = (int (*)())(dlsym(lib,"ws_processCmd"));
	wsdevice->AddWsFd = (ssize_t(*)(int mEpollFd, int OP))(dlsym(lib,"AddWsFd"));
	wsdevice->ConstructSensorbufferFromSocket_input = (int(*)(RawEvent** eventBuffer ,char *recvmsg,int32_t deviceID))(dlsym(lib,"ConstructSensorbufferFromSocket_input"));	
	wsdevice->mapAxisEx = (int(*)(int32_t deviceId, int32_t scanCode, AxisInfo* outAxisInfo,int32_t mJoyCount))(dlsym(lib,"mapAxisEx"));
	wsdevice->getAbsoluteAxisInfoEx = (int(*)(int32_t deviceId, int axis, RawAbsoluteAxisInfo* outAxisInfo,int32_t mJoyCount))(dlsym(lib,"getAbsoluteAxisInfoEx"));
	ret = wsdevice->fun();

	eventItem.data.u32 = EPOLL_ID_SOKCET;
    result = wsdevice->AddWsFd(mEpollFd, EPOLL_CTL_ADD);
    LOG_ALWAYS_FATAL_IF(result != 0, "Could not add socket to epoll instance.  errno=%d",
            errno);
...
status_t EventHub::getAbsoluteAxisInfo(int32_t deviceId, int axis,
    ...
    if (axis >= 0 && axis <= ABS_MAX) {
		...
		if(getAbsoluteAxisInfoEx(deviceId, axis, outAxisInfo)) return OK;
    }
    return -1;
}
...
status_t EventHub::mapAxis(int32_t deviceId, int32_t scanCode, AxisInfo* outAxisInfo) const {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
	if(mapAxisEx(deviceId, scanCode, outAxisInfo)) return NO_ERROR;
    if (device && device->keyMap.haveKeyLayout()) {
...
size_t EventHub::getEvents(int timeoutMillis, RawEvent* buffer, size_t bufferSize) {
    ...
    for (;;) {
        ...
        while (mPendingEventIndex < mPendingEventCount) {
			...
			if(eventItem.data.u32 == EPOLL_ID_SOKCET){
                if (eventItem.events & EPOLLIN) {
					char buffer[1024];
					bzero(buffer,1024);
				    int sk_count = wsdevice->ConstructSensorbufferFromSocket_input(&event,buffer,mMouseDeviceId);		
						ALOGW("<<<<<< WSLIB >>>>>> buffer =%s",buffer);					
					if(strncmp("create",buffer,6)==0){
							createVirtualJoystickLocked();
						}else if (strncmp("destroy",buffer,7)==0)
						{
							...
							if(msg!=NULL)
							{
								...
								{
									index = atoi(msg);
								ALOGW("<<<<<< WSLIB  >>>>>> index=%d ",index);	
								}
							}
						    closeVirtualJoystickLocked(index);
						}else if (strncmp("clearall",buffer,8)==0)
						{
							closeAllVirtualJoystickLocked();
                      }
					capacity -= sk_count;
	                if (capacity == 0) {
...
void EventHub::createVirtualMouseLocked() {
    InputDeviceIdentifier identifier;
    identifier.name = "VirtualMouse";
    identifier.uniqueId = "<virtualMouse>";
    setDescriptor(identifier);

    Device* device = new Device(-1, mNextDeviceId, String8("<virtualMouse>"), identifier);
    device->classes = INPUT_DEVICE_CLASS_CURSOR
				| INPUT_DEVICE_CLASS_KEYBOARD
				| INPUT_DEVICE_CLASS_VIRTUAL;
    addDeviceLocked(device);
	mMouseDeviceId = mNextDeviceId;
	mNextDeviceId++;
}

int array_delete(int index ,int *array,int count)
{
    int i = 0;
    int *p = array;
...
void EventHub:: DumpJoyStickArray()
{
	ALOGW("<<<<<< Inject  >>>>>> mJoyCount = %d",mJoyCount);

    int i = 0;
	for(i =0;i<mJoyCount;i++)
	{
		ALOGW("<<<<<< Inject  >>>>>> mJoyStickId[%d] = %d",i,mJoystickDeviceId[i]);
	}
}

void EventHub::createVirtualJoystickLocked() {
	if(mJoyCount >=10)
	{
		ALOGW("<<<<<< LEE  >>>>>> joystick count over\n");
		return;
	}
    InputDeviceIdentifier identifier;
    ...
    Device* device = new Device(-1, mNextDeviceId, String8("<virtualJoystick>"), identifier);
    device->classes = INPUT_DEVICE_CLASS_JOYSTICK
				|INPUT_DEVICE_CLASS_GAMEPAD
				|INPUT_DEVICE_CLASS_KEYBOARD
				|INPUT_DEVICE_CLASS_EXTERNAL;
    addDeviceLocked(device);
	mJoystickDeviceId[mJoyCount] = mNextDeviceId;
	mNextDeviceId++;
	mJoyCount++;
	ALOGW("WSLIB  after mJoyCount++ EVENTBUB mJoyCount=%d\n",mJoyCount);
	DumpJoyStickArray();
}

void EventHub::closeVirtualJoystickLocked(int index) {
	Device* device = NULL;
	ALOGW("<<<<<< LEE  >>>>>> index  = %d",index);
	if(index < 0)
		return;

	ALOGW("<<<<<< Inject  >>>>>> closeVirtualJoystickLocked %d %d",index,mJoystickDeviceId[index]);
	for (size_t i = 0; i < mDevices.size(); i++)
	{
		device = mDevices.valueAt(i);
		if (device->id == mJoystickDeviceId[index])
		{
			ALOGW("<<<<<< LEE  >>>>>> closeVirtualJoystickLocked i = %d",i);
			closeDeviceLocked(device);
			array_delete(index,mJoystickDeviceId,mJoyCount);
			mJoyCount--;
		}
	}
	DumpJoyStickArray();
...
void EventHub::closeAllVirtualJoystickLocked() {
	int count = mJoyCount;
	int i=0;
	for(i=0;i<count;i++)
	{
	    closeVirtualJoystickLocked(0);
	}
}
